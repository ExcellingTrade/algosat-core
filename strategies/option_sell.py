...
class OptionSellStrategy(StrategyBase):
    ...
    async def process_cycle(self) -> Optional[dict]:
        ...
        if not history_data or all(h is None or getattr(h, 'empty', False) for h in history_data.values()):
            ...
            send_telegram_async("❌🔐 <b>OptionSell: Evaluate Signal skipped</b>\nNo history data received")
            return None
    ...
    async def process_order(self, signal_payload, data, strike):
        ...
        try:
            ...
            if main_all_failed:
                ...
                # Update main order status to ENTRY_FAILED before raising exception
                main_order_id = main_order_result.get('order_id') or main_order_result.get('id')
                if main_order_id:
                    try:
                        logger.info(f"🔄 Updating main order {main_order_id} status to ENTRY_FAILED")
                        await self.order_manager.update_order_status_in_db(main_order_id, "ENTRY_FAILED")
                        logger.info(f"✅ Main order {main_order_id} status updated to ENTRY_FAILED")
                    except Exception as status_e:
                        logger.error(f"⚠️ Failed to update main order {main_order_id} status to ENTRY_FAILED: {status_e}")
    ...
    async def evaluate_signal(self, data, config: dict, strike: str) -> Optional[TradeSignal]:
        ...
        try:
            ...
            if (
                ...
                if config.get("trailing_stoploss_enabled", False) and regime != "sideways":
                    try:
    ...
    async def update_trailing_stoploss(self, order_row, history_df, trade_config):
        ...
        Only activates after price crosses below the original target (favorable direction for SELL).
        """
        try:
            ...
            # Check if trailing stoploss should be activated based on orig_target
            orig_target = order_row.get('orig_target')
            ltp = history_df.iloc[-1].get('close')
            order_id = order_row.get('id')
            
            if orig_target is not None:
                # For SELL trades: only activate trailing stoploss if price has moved below original target
                if ltp >= orig_target:
                    logger.debug(f"🎯 Trailing stoploss for SELL order {order_id} not activated: LTP {ltp} >= orig_target {orig_target} (price hasn't crossed original target)")
                    return
                else:
                    logger.debug(f"🎯 Trailing stoploss for SELL order {order_id} ACTIVATED: LTP {ltp} < orig_target {orig_target} (price crossed below original target)")
            else:
                logger.debug(f"🎯 Trailing stoploss for SELL order {order_id}: orig_target is None, proceeding without target activation check")
            ...
            if stoploss_type == "atr":
                ...
                if current_sl is not None and new_trail_sl < current_sl and ltp < new_trail_sl:
                    ...
                    logger.info(f"🔽 Trailing stop-loss (ATR, SELL) updated for order {order_id}: {current_sl} -> {new_trail_sl} (LTP: {ltp})")
                else:
                    logger.debug(f"🔽 Trailing stop-loss (ATR, SELL) not updated for order {order_id}: new_sl={new_trail_sl}, current_sl={current_sl}, ltp={ltp}")
                ...
                if current_sl is not None and new_trail_sl < current_sl and ltp < new_trail_sl:
                    await self.order_manager.update_order_stop_loss_in_db(order_id, new_trail_sl)
                    logger.info(f"🔽 Trailing stop-loss (Supertrend, SELL) updated for order {order_id}: {current_sl} -> {new_trail_sl} (LTP: {ltp})")
                else:
                    logger.debug(f"🔽 Trailing stop-loss (Supertrend, SELL) not updated for order {order_id}: new_sl={new_trail_sl}, current_sl={current_sl}, ltp={ltp}")
            else:
